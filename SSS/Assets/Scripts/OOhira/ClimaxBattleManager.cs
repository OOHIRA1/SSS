...
public class ClimaxBattleManager : MonoBehaviour
{
    ...
    [SerializeField]
    DoubleDoorCurtain _curtain = null;
    [SerializeField]
    SpriteRenderer _background = null;  //背景画像
    [SerializeField]
    Sprite _detectiveOffice = null; //探偵ラボの背景画像
    [SerializeField]
    Hanamichi _hanamichi = null;
    [SerializeField]
    Detective _detective = null;
    [SerializeField]
    RayShooter _rayshooter = null;
    [SerializeField]
    GameObject _playerLifeUI = null;    //プレイヤーのライフUI
    [SerializeField]
    GameObject _choises = null;     //選択肢群
    [SerializeField]
    Vector3 _detectiveMovePos = new Vector3(0, 0, 0);
    [SerializeField]
    CutinControll[] _cutinControlls = new CutinControll[2];
    bool _cutinFlag;                                                //カットイン演出をしたかどうかのフラグ
    GameObject _criminal = null;                                    //選択した犯人
    [SerializeField]
    Vector3 _npcAppearPos = new Vector3(0, 0, 0); //選択した犯人の出現する座標
    Butler _butler = null;
    [SerializeField]
    Vector3 _butlerMovePos = new Vector3(0, 0, 0);    //執事の移動先座標
    [SerializeField]
    Effect _effectQuestion = null;                  //ハテナエフェクト
    bool _questionEffectAppearFlag;                                 //ハテナエフェクトをしたかどうかのフラグ
    bool _startfallingFlag;                                                 //落ち始めたかどうかのフラグ
    [SerializeField]
    FalledTrigger _falledTrigger = null;
    [SerializeField]
    ScenesManager _scenesManager = null;
    [SerializeField]
    Image _fadeOutPanel = null;
    [SerializeField]
    float _fadeOutSpeed = 0;    //暗転処理のスピード(alpha/second) 
    ...
    {
        if (forcedNonActiveFlag)
        {   //stateによらずforcedNonActiveFlagがtrueなら強制的に非アクティブにする
            ui.SetActive(false);
            return;
        }
        if (forcedActieFlag)
        {       //stateによらずforcedActiveFlagがtrueなら強制的にアクティブにする
            ui.SetActive(true);
            return;
        }
    ...
    {
        if (_gameDataManager.GetCriminal() == "Butler" && _gameDataManager.GetDangerousWeapon() == "DangerousWepon3")
        {  //選んだものが正しいかの確認
